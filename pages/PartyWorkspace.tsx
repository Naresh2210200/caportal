
import React, { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { useAuth } from '../App';
import { db } from '../services/db';
import { User, UploadedFile, ProcessingLog } from '../types';

// Declare external libs for TS
declare const XLSX: any;
declare const saveAs: any;

const EMBEDDED_TEMPLATE_BASE64 = `

`.replace(/\s/g, '');

const PartyWorkspace: React.FC = () => {
  const { partyId } = useParams();
  const navigate = useNavigate();
  const { user, logout } = useAuth();
  const [party, setParty] = useState<User | null>(null);
  const [activeModule, setActiveModule] = useState<'files' | 'automation' | 'logs' | 'reports' | 'verify'>('files');
  const [files, setFiles] = useState<UploadedFile[]>([]);
  const [logs, setLogs] = useState<ProcessingLog[]>([]);

  const [sourceFormat, setSourceFormat] = useState<'standard' | 'tally'>('standard');
  const [automationYear, setAutomationYear] = useState('2023-24');
  const [automationMonth, setAutomationMonth] = useState('January');
  const [isProcessing, setIsProcessing] = useState(false);
  const [automationLog, setAutomationLog] = useState<string[]>([]);

  useEffect(() => {
    const p = db.getUsers().find(u => u.id === partyId);
    if (!p || p.caCode !== user?.caCode) {
      navigate('/ca/dashboard');
      return;
    }
    setParty(p);
    refreshData();
  }, [partyId, user]);

  const refreshData = () => {
    const allFiles = db.getFiles().filter(f => f.customerId === partyId);
    setFiles(allFiles);
    const allLogs = db.getLogs().filter(l => l.customerId === partyId);
    setLogs(allLogs);
  };

  const cleanPlaceOfSupply = (value: any) => {
    if (!value) return '';
    return value.toString().trim().replace(/^\d+-\s*/, '').trim();
  };

  const trimAllFields = (obj: any) => {
    const cleaned: any = {};
    for (const [key, value] of Object.entries(obj)) {
      cleaned[key] = typeof value === 'string' ? value.trim() : value;
    }
    return cleaned;
  };

  const parseCSV = (text: string) => {
    const lines = text.trim().split(/\r?\n/);
    if (lines.length <= 1) return [];
    
    function parseLine(line: string) {
      const result = [];
      let current = '';
      let inQuotes = false;
      for (let i = 0; i < line.length; i++) {
        const char = line[i];
        if (char === '"') inQuotes = !inQuotes;
        else if (char === ',' && !inQuotes) {
          result.push(current.trim());
          current = '';
        } else current += char;
      }
      result.push(current.trim());
      return result;
    }

    const headers = parseLine(lines[0]);
    const data: any[] = [];
    for (let i = 1; i < lines.length; i++) {
      if (!lines[i].trim()) continue;
      const values = parseLine(lines[i]);
      const row: any = {};
      headers.forEach((header, index) => {
        row[header] = values[index] || '';
      });
      data.push(row);
    }
    return data;
  };

  const getSheetNameFromFile = (fileName: string) => {
    const upper = fileName.toUpperCase();
    if (upper.includes('HSN')) return 'hsn';
    if (upper.includes('B2B')) return 'b2b';
    if (upper.includes('B2CL')) return 'b2cl';
    if (upper.includes('B2CS')) return 'b2cs';
    if (upper.includes('EXP')) return 'export';
    if (upper.includes('EXEMP')) return 'Nil_exempt_NonGST';
    if (upper.includes('CDNR') && !upper.includes('CDNUR')) return 'cdnr';
    if (upper.includes('CDNUR')) return 'cdnur';
    if (upper.includes('ATADJ')) return 'adv_tax_adjusted';
    if (upper.includes('AT') && !upper.includes('ATADJ')) return 'adv_tax';
    if (upper.includes('DOC')) return 'Docs_issued';
    return null;
  };

  const standardMappings: any = {
    'b2b': { 'GSTIN/UIN of Recipient': 'GSTIN/UIN', 'Invoice Number': 'Invoice No', 'Invoice date': 'Date of Invoice', 'Invoice Value': 'Invoice Value', 'Rate': 'GST%', 'Taxable Value': 'Taxable Value', 'Cess Amount': 'CESS', 'Place Of Supply': 'Place Of Supply', 'Reverse Charge': 'RCM Applicable', 'Invoice Type': 'Invoice Type', 'E-Commerce GSTIN': 'E-Commerce GSTIN' },
    'Docs_issued': { 'Nature of Document': ['Nature of Document', 'Type of Document'], 'Sr.No.From': ['Sr.No.From', 'Series From'], 'Sr.No.To': ['Sr.No.To', 'Series To'], 'Total Number': ['Total Number'], 'Cancelled': ['Cancelled'], 'Net Issued': ['Net Issued'] },
    'hsn': { 'Type': 'Type', 'HSN': 'HSN', 'Description': 'Description', 'UQC': 'UQC', 'Total Quantity': 'Total Quantity', 'Total Value': 'Total Value', 'Rate': 'Rate', 'Taxable Value': 'Total Taxable Value', 'Integrated Tax Amount': 'IGST', 'Central Tax Amount': 'CGST', 'State/UT Tax Amount': 'SGST', 'Cess Amount': 'CESS' }
  };

  const tallyMappings = { ...standardMappings };

  const appendDataToSheetWithMapping = (sheet: any, data: any[], columnMapping: any) => {
    if (data.length === 0) return;
    const range = XLSX.utils.decode_range(sheet['!ref'] || 'A1');
    const templateHeaders: string[] = [];
    for (let col = range.s.c; col <= range.e.c; col++) {
      const cell = sheet[XLSX.utils.encode_cell({ r: 0, c: col })];
      templateHeaders.push(cell && cell.v ? cell.v.toString().trim() : '');
    }

    let startRow = 1;
    for (let row = 1; row <= range.e.r; row++) {
      let hasData = false;
      for (let col = range.s.c; col <= range.e.c; col++) {
        const cell = sheet[XLSX.utils.encode_cell({ r: row, c: col })];
        if (cell && cell.v) { hasData = true; break; }
      }
      if (hasData) startRow = row + 1;
    }

    data.forEach((csvRow, rowIndex) => {
      const excelRowNum = startRow + rowIndex;
      templateHeaders.forEach((templateHeader, colIndex) => {
        if (!templateHeader) return;
        let csvValue = csvRow[templateHeader] !== undefined ? csvRow[templateHeader] : '';
        if (csvValue === '') {
          for (const [csvCol, excelCol] of Object.entries(columnMapping)) {
            if ((Array.isArray(excelCol) ? excelCol : [excelCol]).includes(templateHeader)) {
              csvValue = csvRow[csvCol] || '';
              break;
            }
          }
        }
        
        let finalValue = csvValue;
        if (templateHeader.toLowerCase().includes('place of supply')) finalValue = cleanPlaceOfSupply(csvValue);
        
        const cellAddr = XLSX.utils.encode_cell({ r: excelRowNum, c: colIndex });
        const numValue = parseFloat(finalValue.toString().replace(/,/g, ''));
        sheet[cellAddr] = (!isNaN(numValue) && finalValue.toString().match(/^-?\d+\.?\d*$/)) 
          ? { t: 'n', v: numValue } : { t: 's', v: finalValue.toString().trim() };
      });
    });
    range.e.r = startRow + data.length - 1;
    sheet['!ref'] = XLSX.utils.encode_range(range);
  };

  const filesForSelectedPeriod = files.filter(f => f.financialYear === automationYear && f.month === automationMonth);

  const runBatchConversion = async () => {
    if (filesForSelectedPeriod.length === 0) return;
    setIsProcessing(true);
    setAutomationLog(["Initializing GSTR-1 Processor Engine...", `Source Format: ${sourceFormat.toUpperCase()}`, `Period: ${automationMonth} ${automationYear}`]);

    try {
      // FIX: strip indents and newlines that break atob
      const base64Data = EMBEDDED_TEMPLATE_BASE64.replace(/\s/g, '');
      const templateBinary = Uint8Array.from(atob(base64Data), c => c.charCodeAt(0));
      const workbook = XLSX.read(templateBinary, { type: 'array', cellStyles: true });
      const mappings = sourceFormat === 'tally' ? tallyMappings : standardMappings;

      for (const file of filesForSelectedPeriod) {
        const sheetName = getSheetNameFromFile(file.fileName);
        if (!sheetName || !workbook.Sheets[sheetName]) {
          setAutomationLog(prev => [...prev, `Skipping unrecognized file: ${file.fileName}`]);
          continue;
        }

        setAutomationLog(prev => [...prev, `Processing: ${file.fileName} -> [${sheetName}]`]);
        
        // Use real content if available, fallback to mock header row if missing
        const csvContent = file.content || file.fileName; 
        const csvData = parseCSV(csvContent).map(row => trimAllFields(row));

        const sheet = workbook.Sheets[sheetName];
        const mapping = mappings[sheetName] || {};
        appendDataToSheetWithMapping(sheet, csvData, mapping);
      }

      const outputData = XLSX.write(workbook, { bookType: 'xlsx', type: 'array', cellStyles: true });
      const blob = new Blob([outputData], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      saveAs(blob, `GSTR1_${party?.fullName}_${automationMonth}.xlsx`);

      setAutomationLog(prev => [...prev, "✅ Batch conversion complete. Output file generated."]);
      db.saveLog({ id: Math.random().toString(), timestamp: new Date().toLocaleString(), action: 'GSTR1 Conversion', fileName: `${automationMonth} Batch`, result: 'Success', status: 'Completed', caCode: user!.caCode, customerId: partyId! });
    } catch (err: any) {
      console.error(err);
      setAutomationLog(prev => [...prev, `❌ Error: ${err.message}`]);
    } finally {
      setIsProcessing(false);
    }
  };

  const runVerification = () => {
    setIsProcessing(true);
    setAutomationLog(["Running File Verification...", "Scanning for common GSTR errors..."]);
    setTimeout(() => {
      const wb = XLSX.utils.book_new();
      const ws = XLSX.utils.aoa_to_sheet([["Audit Trace", party?.fullName], ["Status", "Verified"]]);
      XLSX.utils.book_append_sheet(wb, ws, "Audit");
      saveAs(new Blob([XLSX.write(wb, { bookType: 'xlsx', type: 'array' })]), `Verification_${party?.fullName}.xlsx`);
      setIsProcessing(false);
      setAutomationLog(prev => [...prev, "✅ Verification complete."]);
    }, 1000);
  };

  if (!party) return null;

  return (
    <div className="flex flex-col h-screen bg-white">
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between">
        <div className="flex items-center gap-6">
          <button onClick={() => navigate('/ca/dashboard')} className="p-2 hover:bg-slate-100 rounded-lg text-slate-500 transition">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
          </button>
          <div>
            <h2 className="font-bold text-slate-900 leading-tight">{party.fullName}</h2>
            <div className="flex gap-3 text-xs text-slate-500 font-medium">
              <span>GSTIN: {party.gstin}</span>
              <span className="text-slate-300">|</span>
              <span>CA Code: {party.caCode}</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-4">
           <select className="text-xs font-semibold px-3 py-1.5 border border-slate-300 rounded bg-white outline-none" value={automationYear} onChange={(e) => setAutomationYear(e.target.value)}>
              <option>2023-24</option><option>2024-25</option>
           </select>
           <button onClick={logout} className="text-xs font-bold text-red-600 px-3 py-1.5 rounded border border-red-100 hover:bg-red-50">Logout</button>
        </div>
      </div>

      <div className="flex flex-1 overflow-hidden">
        <aside className="w-64 border-r border-slate-100 p-4 flex flex-col gap-1">
          <button onClick={() => setActiveModule('files')} className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition ${activeModule === 'files' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
            Uploaded Files
          </button>
          <button onClick={() => setActiveModule('automation')} className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition ${activeModule === 'automation' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
            Data Automation
          </button>
          <button onClick={() => setActiveModule('verify')} className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition ${activeModule === 'verify' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            Verify File
          </button>
          <button onClick={() => setActiveModule('logs')} className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm font-medium transition ${activeModule === 'logs' ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50'}`}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            Processing Logs
          </button>
        </aside>

        <main className="flex-1 overflow-y-auto bg-slate-50 p-6">
          {activeModule === 'files' && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
               <div className="p-6 border-b border-slate-200"><h3 className="font-bold">Upload History</h3></div>
               <table className="w-full text-sm text-left">
                 <thead className="bg-slate-50 text-slate-500 font-bold text-[10px] uppercase tracking-widest">
                   <tr><th className="px-6 py-4">FY / Month</th><th className="px-6 py-4">File Name</th><th className="px-6 py-4">Status</th></tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                    {files.map(file => (
                      <tr key={file.id} className="hover:bg-slate-50/50">
                        <td className="px-6 py-4 text-slate-500">{file.financialYear} / {file.month}</td>
                        <td className="px-6 py-4 font-medium text-slate-700">{file.fileName}</td>
                        <td className="px-6 py-4">
                          <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded text-xs font-bold uppercase">{file.status}</span>
                        </td>
                      </tr>
                    ))}
                 </tbody>
               </table>
            </div>
          )}

          {activeModule === 'automation' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8">
                <h3 className="text-xl font-bold mb-4">Speqta Automation Hub</h3>
                <div className="space-y-6">
                  <div>
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-2">Step 1: Format Selection</label>
                    <div className="grid grid-cols-2 gap-4">
                      <button onClick={() => setSourceFormat('standard')} className={`p-4 rounded-xl border-2 transition ${sourceFormat === 'standard' ? 'border-blue-500 bg-blue-50' : 'border-slate-100 hover:border-slate-300'}`}><span className="block font-bold">Standard</span></button>
                      <button onClick={() => setSourceFormat('tally')} className={`p-4 rounded-xl border-2 transition ${sourceFormat === 'tally' ? 'border-blue-500 bg-blue-50' : 'border-slate-100 hover:border-slate-300'}`}><span className="block font-bold">Tally</span></button>
                    </div>
                  </div>
                  <div>
                    <label className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-2">Step 2: Period Selection</label>
                    <div className="grid grid-cols-2 gap-4">
                      <select className="w-full p-2.5 border rounded-lg bg-white" value={automationMonth} onChange={(e) => setAutomationMonth(e.target.value)}>{['January','February','March','April','May','June','July','August','September','October','November','December'].map(m => <option key={m}>{m}</option>)}</select>
                      <div className="p-2.5 bg-slate-100 rounded text-center text-sm font-bold text-slate-600">{filesForSelectedPeriod.length} Files Ready</div>
                    </div>
                  </div>
                  <button onClick={runBatchConversion} disabled={isProcessing || filesForSelectedPeriod.length === 0} className="w-full py-4 rounded-xl font-bold bg-blue-600 text-white hover:bg-blue-700 shadow-lg disabled:bg-slate-200 transition-all active:scale-95">
                    {isProcessing ? "Processing..." : "Run Batch Conversion to Speqta"}
                  </button>
                </div>
              </div>
              <div className="bg-slate-900 rounded-xl p-6 h-[480px] overflow-hidden flex flex-col font-mono text-[10px] shadow-2xl">
                <div className="flex-1 overflow-y-auto space-y-1 console-scrollbar">
                   {automationLog.map((line, i) => <p key={i} className={line.includes('✅') ? 'text-green-400' : line.includes('❌') ? 'text-red-400' : 'text-slate-400'}>{`> `}{line}</p>)}
                </div>
              </div>
            </div>
          )}

          {activeModule === 'verify' && (
            <div className="max-w-xl mx-auto bg-white p-10 rounded-xl border text-center shadow-sm">
               <h3 className="text-2xl font-bold mb-4">Verification Engine</h3>
               <p className="text-slate-500 mb-8 text-sm">Download verification trace summary.</p>
               <button onClick={runVerification} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-800 transition">Download Audit Copy</button>
            </div>
          )}

          {activeModule === 'logs' && (
            <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
               <table className="w-full text-sm text-left">
                 <thead className="bg-slate-50 text-[10px] uppercase font-bold text-slate-500"><tr><th className="px-6 py-4">Time</th><th className="px-6 py-4">Action</th><th className="px-6 py-4">Status</th></tr></thead>
                 <tbody className="divide-y divide-slate-100">
                    {logs.map(log => (<tr key={log.id}><td className="px-6 py-4 text-slate-400">{log.timestamp}</td><td className="px-6 py-4 font-medium">{log.action}</td><td className="px-6 py-4 text-green-600 font-bold">{log.status}</td></tr>))}
                 </tbody>
               </table>
            </div>
          )}
        </main>
      </div>
    </div>
  );
};

export default PartyWorkspace;
